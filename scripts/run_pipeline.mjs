 ...
 *   node scripts/run_pipeline.mjs [base_url] [--batch=800] [--no-insights] [--days=7] [--to=<ISO>]
...
const TO = (args.find((a) => a.startsWith("--to=")) || "").slice(5) || undefined;   // 可選：期間終點／漏斗的「現在」；沒給就由伺服器用資料末端（資料最後一天）
...
async function batches(path, label) {
  ...
  for (;;) {
    const r = await api(path, { batch: BATCH, cursor, now: TO });
...
if (!NO_INS) { const i = await api("/api/insights/run", { days: DAYS, to: TO }); console.log("洞察＋簡報：", JSON.stringify({ ok: i.ok, persisted: i.persisted, narrate: i.narrate, brief: i.brief, ms: i.ms, err: i.body })); }
await api("/api/admin/warm", {});   // 兩種基準（資料截至／到今天）× 7/14/30 的分析／員工效能／決策中心都先算好進持久快取
