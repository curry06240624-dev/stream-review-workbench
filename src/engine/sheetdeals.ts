 ...
 *   - 售出（vehicles.stock_status='sold'）        → deals.status='sold'
 *   - 收訂／送貸／過件（stock_status='reserved'） → deals.status='reserved'（收訂中：訂金已收、貸款送件或過件，還沒交車）
 ...
 *   同一台車已經有貼文／對話來的成交或收訂 → 車源表不再產生，避免重複計算。
 *   收訂後車又變回在庫 → 收訂中那筆刪掉（訂金退了或貸款沒過，車源表沒說原因，不猜）。
 */
...
export async function syncSheetDeals(db: DbLike, opts: { now: string }): Promise<SheetDealsReport> {
  ...
  for (const v of vehicles) {
    ...
    if (st !== "sold" && st !== "reserved") {
      if (mine && str(mine["status"]) === "reserved") { await db.run("DELETE FROM deals WHERE id = ?", num(mine["id"])); rep.removed++; }
      continue;
    }
    const other = await db.first("SELECT id FROM deals WHERE vehicle_id = ? AND status IN ('sold','reserved') AND source_system <> 'sheet' LIMIT 1", vid);
    ...
    const loan = /過件/.test(statusText) ? "approved" : /倒件|沒過|退件/.test(statusText) ? "rejected" : "";
    const status = st === "sold" ? "sold" : "reserved";
    ...
    if (!mine) {
      await db.run(
        `INSERT INTO deals (lead_id, contact_id, staff_id, vehicle_id, status, sale_price, cost, gross_profit, lost_reason, closed_at, external_key, source_system,
                            plate, customer_ref, deposit, loan_status, delivery_by, reported_by, source_kind, peer_dealer, cost_source, gp_is_estimate, report_id, price_source, sheet_status)
         VALUES (NULL, NULL, ?, ?, ?, ?, ?, ?, '', ?, ?, 'sheet', ?, '', '', ?, '', '', 'stock', '', ?, ?, NULL, ?, ?)`,
        staffId, vid, status, price, cost, gp, opts.now, extKey, str(v["plate"]), loan, costSource, costKnown ? 1 : 0, priceSource, sheetStatus);
      rep.created++;
    } else {
      const becameSold = str(mine["status"]) === "reserved" && status === "sold";
      await db.run(
        `UPDATE deals SET status = ?, sale_price = ?, cost = ?, gross_profit = ?, cost_source = ?, gp_is_estimate = ?, price_source = ?, staff_id = COALESCE(staff_id, ?),
                          plate = ?, loan_status = ?, sheet_status = ?, closed_at = CASE WHEN ? = 1 THEN ? ELSE closed_at END WHERE id = ?`,
        status, price, cost, gp, costSource, costKnown ? 1 : 0, priceSource, staffId, str(v["plate"]), loan, sheetStatus, becameSold ? 1 : 0, opts.now, num(mine["id"]));
      rep.updated++;
    }
  }
  return rep;
