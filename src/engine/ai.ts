...
export function factsPack(a: Analytics, insights: Row[], fixed?: string): { text: string; allowed: Set<string> } {
  ...
  let lines = [
    ...
    `有日期的成交 ${a.deals.sold} 台（前期 ${a.deals.prev_sold}）、售價合計 ${Math.round(a.deals.revenue / 10000)} 萬（其中同行車 ${a.deals.peer_sold} 台）；毛利只算得出 ${a.deals.gp_known} 台＝${Math.round(a.deals.gross_profit / 10000)} 萬${a.deals.gp_unknown ? `，另 ${a.deals.gp_unknown} 台沒有成本（同行車與已賣掉、不在車源表的車），所以「毛利 ${Math.round(a.deals.gross_profit / 10000)} 萬」「毛利率 ${pct(a.deals.gp_margin)}」不是全月毛利，簡報只能寫「只有 ${a.deals.gp_known} 台算得出毛利」` : `、毛利率 ${pct(a.deals.gp_margin)}`}、低於成本 ${a.deals.below_cost} 筆${a.deals.sheet_sold ? `（其中 ${a.deals.sheet_sold} 筆來自車源表${a.deals.sheet_vanished ? `，${a.deals.sheet_vanished} 筆是車從車源表消失推定的` : ""}）` : ""}；成交裡 ${a.deals.undelivered} 台還沒交車（車源表 收訂／送貸／過件，${Math.round(a.deals.undelivered_amount / 10000)} 萬；收訂就算成交）`,
    `車源表目前售出／收訂 ${a.deals.undated?.n ?? 0} 台、金額 ${Math.round((a.deals.undated?.amount ?? 0) / 10000)} 萬：第一次匯入就是這個狀態，車源表沒有成交日，所以不算本期成交、也不能說是「本期賣了」${a.deals.sold === 0 && (a.deals.undated?.n ?? 0) > 0 ? "；本期「有日期的成交 0」是因為系統還沒有成交日期資料（成交群還沒匯入），不是真的沒賣車，簡報不要寫「成交掛零」「沒有成交」，要寫「系統還沒有成交日期」" : ""}`,
    `SABC 分級（系統推算，近 30 天有訊息的未結案客戶）：S ${a.grades?.open?.["S"] ?? 0}、A ${a.grades?.open?.["A"] ?? 0}、B ${a.grades?.open?.["B"] ?? 0}、C ${a.grades?.open?.["C"] ?? 0}；長週期 ${a.grades?.long_cycle ?? 0}；本期新進線 S ${a.grades?.period?.["S"] ?? 0}、A ${a.grades?.period?.["A"] ?? 0}、B ${a.grades?.period?.["B"] ?? 0}、C ${a.grades?.period?.["C"] ?? 0}`,
...
function templateBrief(a: Analytics, insights: Row[], fixed?: string): BriefContent {
  ...
  return {
    happened: `${fixed ? `整個 ${Number(fixed.slice(5, 7))} 月` : `最近 ${a.period.days} 天`}新進線 ${a.funnel.leads} 位（另 ${a.funnel.menu_only_leads} 位只點選單），業務報價 ${a.price_dropoff.base} 次，有日期的成交 ${a.deals.sold} 台、售價合計 ${Math.round(a.deals.revenue / 10000)} 萬${a.deals.gp_unknown ? `（毛利只有 ${a.deals.gp_known} 台算得出）` : `、毛利 ${Math.round(a.deals.gross_profit / 10000)} 萬`}。`,
