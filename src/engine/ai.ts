...
export function factsPack(a: Analytics, insights: Row[]): { text: string; allowed: Set<string> } {
  ...
  const lines = [
    ...
    `成交 ${a.deals.sold} 台（前期 ${a.deals.prev_sold}）、營收 ${Math.round(a.deals.revenue / 10000)} 萬、毛利 ${Math.round(a.deals.gross_profit / 10000)} 萬、毛利率 ${pct(a.deals.gp_margin)}、低於成本 ${a.deals.below_cost} 筆`,
