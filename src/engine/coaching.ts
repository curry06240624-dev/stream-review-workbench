...
export async function computeDecisions(db: DbLike, report: StaffReport, now: string): Promise<DecisionCard[]> {
  ...
  const noCost = await db.first("SELECT COUNT(*) AS n, COALESCE(SUM(sale_price),0) AS amount FROM deals WHERE status = 'sold' AND cost_source = 'none' AND COALESCE(closed_at_source,'') <> 'import' AND closed_at >= ? AND closed_at < ?", report.period.from, report.period.to);
  // 車源表第一次匯入就是售出／收訂的：沒有成交日 → 不算本期、算不出各業務本期成交；這是資料流的問題，要請訊息組補欄位
  const undated = await db.first("SELECT COUNT(*) AS n, COALESCE(SUM(sale_price),0) AS amount, COALESCE(SUM(CASE WHEN delivered = 0 THEN 1 ELSE 0 END),0) AS reserved FROM deals WHERE status = 'sold' AND closed_at_source = 'import'");
  if (num(undated?.["n"]) >= 1) cards.push({ key: "undated_deals", priority: "high", kind: "review_process", title: `${num(undated?.["n"])} 台車源表售出／收訂沒有成交日期`, why: `合計 ${wan(num(undated?.["amount"]))}（${num(undated?.["reserved"])} 台還沒交車）。車源表沒有「收訂日期」欄，這些是第一次匯入就已經是售出／收訂，所以系統不知道是哪一天成交 → 不算本期成交、各業務的本期成交也算不出來`, observed: "總覽與成交頁把它們另列為「車源表目前售出／收訂，日期不明」", action: "請訊息組在車源表加一欄「收訂日期」（改狀態那天就填）；成交群匯出進來後送貨囉貼文會自動對上並補日期", measure: "有日期的成交筆數", metric_key: "", staff_ids: [], links: [{ label: "成交與毛利", href: "/deals" }, { label: "資料上傳", href: "/uploads" }], claim: "fact" });
  ...
  // 4d. 成交價低於車源表「調作價」（實賣價）：不是資料問題，是折讓超過公司訂的實賣價、或車源表沒更新
  const below = await db.all("SELECT d.staff_id, d.sale_price, v.sell_price, COALESCE(u.name,'') AS staff, COALESCE(v.brand||' '||v.model,'') AS car FROM deals d JOIN vehicles v ON v.id = d.vehicle_id LEFT JOIN users u ON u.id = d.staff_id WHERE d.status = 'sold' AND v.sell_price > 0 AND d.sale_price > 0 AND d.sale_price < v.sell_price AND COALESCE(d.closed_at_source,'') <> 'import' AND d.closed_at >= ? AND d.closed_at < ?", report.period.from, report.period.to);
  if (below.length) cards.push({ key: "below_sell_price", priority: below.length >= 3 ? "high" : "medium", kind: "review_pricing", title: `${below.length} 筆成交低於車源表的調作價`, why: `${below.slice(0, 4).map((r) => `${String(r["car"])} 調作價 ${wan(num(r["sell_price"]))}、成交 ${wan(num(r["sale_price"]))}${String(r["staff"]) ? `（${String(r["staff"])}）` : ""}`).join("；")}；合計比調作價少收 ${wan(below.reduce((s, r) => s + num(r["sell_price"]) - num(r["sale_price"]), 0))}`, observed: "調作價是公司訂的實賣價；低於它成交代表折讓超出授權，或車源表的調作價沒跟著更新", action: "逐筆問清楚是誰核准的折讓；調作價若已調整請同步更新車源表", measure: "下月低於調作價成交的筆數", metric_key: "", staff_ids: [...new Set(below.map((r) => num(r["staff_id"])).filter(Boolean))], links: [{ label: "成交與毛利", href: "/deals" }], claim: "fact" });
  ...
  // 5. 急迫客戶沒人回
  const stale = await db.first(`SELECT COUNT(*) AS n FROM leads l JOIN funnel_events h ON h.lead_id = l.id AND h.type = 'HIGH_INTENT' WHERE l.outcome = '' AND h.at >= ?
    AND (SELECT MAX(NULLIF(cv.last_staff_at,'')) FROM conversations cv WHERE cv.lead_id = l.id) < ?`, new Date(Date.parse(now) - 14 * D).toISOString(), new Date(Date.parse(now) - D).toISOString());   // 只看最近 14 天說急的
  if (num(stale?.["n"]) >= 3) cards.push({ key: "stale_intent", priority: "high", kind: "contact_leads", title: `${num(stale?.["n"])} 位最近兩週說急的客戶，超過 24 小時沒有業務訊息`, why: "急迫客戶等越久，回來的機率越低", observed: obsText("proposed_after_intent", "表現最佳組在客戶表達急迫後多半立刻約看車"), action: "今天把這幾位分回給業務，下班前回報", measure: "24 小時內回覆率", metric_key: "stale_intent", staff_ids: [], links: [{ label: "需要注意", href: "/attention?kind=high_intent_no_followup" }], claim: "fact" });
  // 6. 值得教全隊的成功模式
