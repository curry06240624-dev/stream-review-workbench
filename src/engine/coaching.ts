...
export async function computeDecisions(db: DbLike, report: StaffReport, now: string): Promise<DecisionCard[]> {
  ...
  const below = await db.all("SELECT d.staff_id, d.sale_price, v.sell_price, COALESCE(u.name,'') AS staff, COALESCE(v.brand||' '||v.model,'') AS car FROM deals d JOIN vehicles v ON v.id = d.vehicle_id LEFT JOIN users u ON u.id = d.staff_id WHERE d.status = 'sold' AND v.sell_price > 0 AND d.sale_price > 0 AND d.sale_price < v.sell_price AND d.closed_at >= ? AND d.closed_at < ?", report.period.from, report.period.to);
