...
const RE = {
  price:      /(?:開價|售價|車價|賣|這台|這款|這個|活動價?|折價完?|優惠價?|價格|價錢|給你|算你)[^\d\n]{0,6}(?<![程跑清])(\d{1,3}(?:\.\d)?)\s*(?:多|幾)?\s*萬(?!\s*(?:公里|km))|(\d{1,3}(?:\.\d)?)\s*(?:多|幾)?\s*萬\s*(?:含|送|這台|可以牽|就可以|就能|交車|全包|含稅|含過戶|而已|就好)|開\s*\d{2,3}(?:\.\d)?(?!\d)|(?:開價|售價|折價完?|活動(?:價|是)|優惠價?)\s*(?:在|是|大概|約|完)?\s*\d{2,3}(?:\.\d)?(?!\d)|(?:月繳|月付|總價|報價|含過戶)\s*(?:大概|約|抓|款)?\s*[\d一二三四五六七八九十]|NT\$\s*\d|\$\s*\d{2,3},\d{3}|\d{2,3}多(?!少|久|台|人|次)/,   // 「月繳」「總價」要跟著數字才算報價（真資料：「預算或月繳款抓多少呢」不是報價）
