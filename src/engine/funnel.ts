...
const RE = {
  price:      /(?:開價|售價|車價|賣|這台|這款|這個|活動價?|折價完?|優惠價?|價格|價錢|給你|算你)[^\d\n]{0,6}(?<![程跑清])(\d{1,3}(?:\.\d)?)\s*(?:多|幾)?\s*萬(?!\s*(?:公里|km))|(\d{1,3}(?:\.\d)?)\s*(?:多|幾)?\s*萬\s*(?:含|送|這台|可以牽|就可以|就能|交車|全包|含稅|含過戶|而已|就好)|開\s*\d{2,3}(?:\.\d)?(?!\d)|(?:開價|售價|折價完?|活動(?:價|是)|優惠價?)\s*(?:在|是|大概|約|完)?\s*\d{2,3}(?:\.\d)?(?!\d)|(?:月繳|月付|總價|報價|含過戶)\s*(?:大概|約|抓|款|是)?\s*(?!一樣|一定|一起|一下|一點|一直|一次|一半)[\d一二三四五六七八九十]|NT\$\s*\d|\$\s*\d{2,3},\d{3}|\d{2,3}多(?!少|久|台|人|次)/,   // 「月繳」「總價」要跟著數字才算報價（真資料：「預算或月繳款抓多少呢」不是報價）
  numWan:     /(\d{1,3}(?:\.\d)?)\s*萬|開\s*(\d{2,3}(?:\.\d)?)(?!\d)/,
  priceAsk:   /(?:萬|元|千)[^\d]{0,4}嗎|預算|接受嗎|多少|是嗎|呢\s*[?？!！]*$|[?？]\s*$|以內|內的|現金內|貼\s*\d|私分|初估|收回來|如果是\s*\d|年份/,   // 問預算的句子不是報價：「25萬內嗎」「月繳9000內都可以接受嗎」「貸款10萬左右這樣嗎」
...
export function detectEvents(ctx: Ctx, vehicles: VehicleName[], now: number): Detected[] {
  ...
  const fin = cust.find((m) => RE.financing.test(m.text) && RE.financingAsk.test(m.text));
  if (fin) {
