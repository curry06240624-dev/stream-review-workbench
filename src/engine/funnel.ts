...
const RE = {
  price:      /(\d{1,3}(?:\.\d)?)\s*萬|開\s*\d{2,3}(?:\.\d)?(?!\d)|(?:月繳|月付|總價|報價|含過戶)\s*(?:大概|約|抓|款)?\s*[\d一二三四五六七八九十]|NT\$\s*\d|\$\s*\d{2,3},\d{3}|\d{2,3}多(?!少|久|台|人|次)/,   // 「月繳」「總價」要跟著數字才算報價（真資料：「預算或月繳款抓多少呢」不是報價）
  ...
  apptProp:   /約個時間|方便嗎|有空嗎|來店|來看車|幫(?:你|您)留車|留車給|哪天有空|來看實車|過來看|現場看|來現場|給你地址|載你|可以看車/,   // 「有空來看看嗎」是跟進不是約時間，不放「來看看」；「保留車款」含「留車」所以只認「幫你留車」
  ...
  soldStaff:  /恭喜(?:您|你)?(?:牽|交|買|入手|成為)|交車(?:愉快|完成|順利|囉|啦|了)|(?:今天|剛剛|昨天|順利)交車|過戶完成|已交車/,   // 真資料的「交車」多半是「交車前會檢查」這種未來式，不算；「恭喜」單獨出現另外再擋（見 soldStaffNot）
  soldStaffNot: /交車前|到交車|交車流程|交車時|交車的|交車那|交車後|恭喜(?:你|您)?(?:呀|啊)?[，,]?\s*(?:我|那|這|但)/,
