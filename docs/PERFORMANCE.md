...
4. **持久快取**（`src/db.js cached()`）：記憶體 → SQLite `cache_json` → 重算。資料一改（匯入／漏斗／分析／分級／配對／行動）就 `bust()` 清兩層。TTL 24 小時。快取鍵：預設基準「資料截至」時鍵＝資料末端 ISO（資料沒變鍵就不變，cron 暖快取幾乎免費）；「到今天」時鍵按台灣日期換（`bucket()`）。`warmCaches` 順手刪過期列。
