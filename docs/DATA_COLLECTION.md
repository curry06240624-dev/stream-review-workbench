...
**給綠化（車源表）**
> 綠化，車源表我想用你說的 Google Sheets API＋服務帳號的方式唯讀。我建好服務帳號後把 email 給你，麻煩請有權限的人把試算表用「檢視者」分享給那個 email 就好，不會寫入。另外想確認「調作價」是什麼意思、T 欄之後還有哪些欄位、目前狀況能不能改成欄位值而不是列顏色。
