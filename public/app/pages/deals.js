...
export async function render(el, ctx) {
  ...
  const soldCols = [
    ...
    { key: "source_kind", label: "來源", render: (x) => `${x.source_system === "sheet" ? chip(`車源表${x.sheet_status ? " · " + esc(x.sheet_status) : ""}`) : x.source_kind === "peer" ? chip(`同行${x.peer_dealer ? " · " + esc(x.peer_dealer) : ""}`, "amber") : chip("庫存")}${x.delivered === 0 ? " " + chip("未交車", "amber") : ""}` },
