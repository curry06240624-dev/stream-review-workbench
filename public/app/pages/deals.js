...
export async function render(el, ctx) {
  ...
  const reserved = rows.filter((x) => x.status === "reserved"), reservedAmt = reserved.reduce((s, x) => s + (x.sale_price || 0), 0);
  ...
  const reservedNote = reserved.length ? `另有 ${reserved.length} 台收訂中（車源表 收訂／送貸／過件，預計 ${nt(reservedAmt)}）。` : "";
  ...
  const soldCols = [
    ...
    { key: "source_kind", label: "來源", render: (x) => (x.source_system === "sheet" ? chip(`車源表${x.sheet_status ? " · " + esc(x.sheet_status) : ""}`) : x.source_kind === "peer" ? chip(`同行${x.peer_dealer ? " · " + esc(x.peer_dealer) : ""}`, "amber") : chip("庫存")) },
  ...
  const reservedCols = [
    ...
    { key: "staff", label: "業務", render: (x) => esc(x.staff || "未指派") },
    { key: "sale_price", label: "預計售價", num: true, render: (x) => `${nt(x.sale_price)}${x.price_source === "sheet_list" ? ' <span class="faint" title="車源表沒填調作價，先用開價">開價</span>' : ""}` },
    ...
    { key: "loan_status", label: "貸款", render: (x) => LOAN[x.loan_status] || "—" },
    { key: "closed_at", label: "更新", render: (x) => fmtD(x.closed_at) },
  ];
  const staffCols = [
  ...
  el.innerHTML = `<div class="wrap stack">
    ...
    <section class="stats">
      ${stat("成交", `${num(d.sold)} 台`, `${delta(d.sold, d.prev_sold, { fmt: num })}${d.peer_sold ? ` <span class="faint">同行 ${d.peer_sold}</span>` : ""}${d.sheet_sold ? ` <span class="faint">車源表 ${d.sheet_sold}</span>` : ""}`)}
      ${stat("營收", nt(d.revenue), delta(d.revenue, d.prev_revenue, { fmt: nt }))}
      ${stat("收訂中", `${reserved.length} 台`, `<span class="faint">預計 ${nt(reservedAmt)} · 車源表 收訂／送貸／過件</span>`)}
