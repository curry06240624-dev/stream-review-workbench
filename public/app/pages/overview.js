...
export async function render(el, ctx) {
  ...
  el.innerHTML = h`<div class="wrap stack">
    ...
    <section class="panel"><h3>漏斗快照 <span class="sp"></span><a href="/funnel" data-link style="font-weight:400;letter-spacing:0">完整漏斗 ›</a></h3>${raw(funnelStrip(stages, { slim: true, bottleneck }))}
      ${raw(`<div class="faint" style="margin-top:8px;font-size:12px">新進線＝有送過訊息的客戶（打字／照片／貼圖）；另有 ${num(f.menu_only_leads || 0)} 位只加好友／點選單，不算進線 · 報價＝業務親自報價（客戶點車卡不算） · 成交只算有日期的${d.undated?.n ? `；車源表另有 ${d.undated.n} 台售出／收訂，成交日不明` : ""}</div>`)}</section>
