...
export async function render(el, ctx) {
  ...
  const profit = byKind("review_pricing") || (a.ok ? { title: a.deals.below_cost ? `${a.deals.below_cost} 筆成交低於成本` : a.deals.gp_unknown ? `毛利只算得出 ${a.deals.gp_known} 台（${wan(a.deals.gross_profit)}），${a.deals.gp_unknown} 台沒有成本` : `毛利率 ${pct(a.deals.gp_margin)}，毛利 ${wan(a.deals.gross_profit)}`, why: a.deals.below_cost ? "先看這幾筆是讓價換成交還是車況" : a.deals.gp_unknown ? "車源表只有在庫車的成本；賣掉的車要會計給成交成本表才算得出毛利" : "", links: [{ href: "/deals" }] } : null);
