...
const ROUTES = [
  ...
  ["/staff", "staff"], ["/staff/:id", "staffProfile"], ["/decisions", "decisions"], ["/manager", "manager"], ["/loss", "loss"], ["/reconcile", "reconcile"], ["/uploads", "uploads"],
...
async function render() {
  ...
  try {
    ...
    if (me && me.fixed_period) query.days = String(me.fixed_period.days);   // 只有一個月資料的站：每一頁都看整個月，?days= 不理
    await PAGES[m.mod].render(view, { params: m.params, query, me, nav });
    view.scrollTop = 0;
  } catch (e) {
    console.error(e);
...
function paintWho() {
  ...
  window.__dataEnd = me.data_end || null; window.__fixedPeriod = me.fixed_period || null; const mode = anchorMode(); const de = me.data_end ? fmtDT(me.data_end) : null;
  ...
  if (me.fixed_period) { w.innerHTML += `<div class="faint" style="margin-top:6px;font-size:12px;line-height:1.5">資料：${esc(me.fixed_period.label)}（${esc(me.fixed_period.range)}）<br>這個站只有這個月的資料，沒有前期可比</div>`; return; }
